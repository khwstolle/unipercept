...
def test_sample():
    ...
    samples = pointrend.sample(src, points, mode="nearest", align_corners=False)
...
def test_random_points():
    ...
    n_points = 10
    d_coord = 2
    points = pointrend.random_points(src, n_points, d_coord)
...
def test_random_points_with_mask():
    ...
    for mask in (
        ...
        points = pointrend.random_points(src, n_points, d_coord, mask)

        assert points.shape == (1, n_points, d_coord)
        assert points.min() >= 0
        assert points.max() <= 1

        samples = pointrend.sample(mask.float(), points, mode="nearest", align_corners=False)
...
def test_random_points_with_importance():
    ...
    points = pointrend.random_points_with_importance(
        src,
        n_points,
        d_coord,
...
def test_pointrend_bins_by_values(use_batch, mode):
    ...
    bins = pointrend.bins_by_values(values, n_bins, use_batch=use_batch, mode=mode)
...
def test_map_in_bins(use_batch: bool, mode: str):
    ...
    values = torch.rand(n_batch, n_values)
    mask = values > 0.1
    bins = pointrend.bins_by_values(values, n_bins, mask=mask, use_batch=use_batch, mode=mode)

    def masked_mean(t, m):
        t = t.masked_fill(~m, torch.nan)
        return torch.nanmean(t, dim=1, keepdim=True)

    mapped = pointrend.map_in_bins(values, bins, masked_mean)
...
def test_map_in_bins_with_log_values(use_batch: bool, mode: str):
    ...
    mapped = pointrend.map_in_bins(sources, bins, masked_mean, values=values)
...
def test_map_in_bins_with_itemcount(use_batch: bool, mode: str):
    ...
    counts = pointrend.map_in_bins(sources, bins, select_count, values=values)
...
def test_map_in_bins_with_itemcount_masked(use_batch: bool, mode: str):
    ...
    counts = pointrend.map_in_bins(sources, bins, select_count, values=values, mask=mask)
    totals = counts.sum(dim=1)
...
def test_random_points_with_bins(use_batch: bool, mode: str):
    ...
    random_points = pointrend.random_points_with_bins(
