...
try:
    import apex.normalization.fused_layer_norm as apex_ln
except ImportError:
    apex_ln = None
...
class LayerNorm(nn.LayerNorm):
    def __new__(cls, *args, **kwargs):
        kwargs.setdefault("eps", LAYERNORM_DEFAULT_EPS)
        if apex_ln is None:
            return nn.LayerNorm(*args, **kwargs)
        kwargs.setdefault("memory_efficient", LAYERNORM_DEFAULT_MEMORY_EFFICIENT)
        return apex_ln.FusedLayerNorm(*args, **kwargs)
...
class RMSNorm(nn.RMSNorm):
    def __new__(cls, *args, **kwargs):
        ...
        kwargs.setdefault("memory_efficient", LAYERNORM_DEFAULT_MEMORY_EFFICIENT)
        return apex_ln.FusedRMSNorm(*args, **kwargs)


def layer_norm(
    input: Tensor, shapes: Size, weight: Tensor | None, bias: Tensor | None, **kwargs
) -> Tensor:
    kwargs.setdefault("eps", LAYERNORM_DEFAULT_EPS)
    if apex_ln is None:
        return nn.functional.layer_norm(input, shapes, weight, bias, **kwargs)
    kwargs.setdefault("memory_efficient", LAYERNORM_DEFAULT_MEMORY_EFFICIENT)
    if weight is None or bias is None:
        assert weight is None
        assert bias is None
        res = apex_ln.fused_layer_norm(input, shapes, **kwargs)
    else:
        res = apex_ln.fused_layer_norm_affine(input, weight, bias, shapes, **kwargs)
    return T.cast(Tensor, res)
...
def rms_norm(
    ...
    **kwargs,
    ...
    kwargs.setdefault("eps", LAYERNORM_DEFAULT_EPS)
    if apex_ln is None:
        return nn.functional.rms_norm(input, shapes, weight, **kwargs)
    kwargs.setdefault("memory_efficient", LAYERNORM_DEFAULT_MEMORY_EFFICIENT)
    if weight is None:
        res = apex_ln.fused_rms_norm(input, shapes, **kwargs)
    else:
        res = apex_ln.fused_rms_norm_affine(input, weight, shapes, **kwargs)
    return T.cast(Tensor, res)
