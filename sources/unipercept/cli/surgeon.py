...
from ._utils import create_subtemplate, human_readable_size
        ...
        if args.depth > 0:
            ...
            for depth, key, value_type in cat_structure(args.weights):
                if "OrderedDict" in value_type:
                    ...
                    struct.append("\t" * depth + "... (use --items/-i to display more)")
                    cur_items -= 1
                    continue
                else:
                    continue
...
class StatsSubcommand(Subcommand, name="stats"):
    ...
    def main(args: argparse.Namespace):
        ...
        num_weights = f"{len(data):,}"

        logger.info("Found %s weights", num_weights)

        logger.info(
            "Total parameters: %s of which %s trainable",
            f"{total_params:,} ({human_readable_size(total_params)})",
            f"{total_trainable_params:,} ({human_readable_size(total_trainable_params)})",
        )
