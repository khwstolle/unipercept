...
def _is_embedding_weight(
    ...
    return ("embed" in name) or ("pos" in name)
    ...
    for module_name, module in model.named_modules():
        for module_param_name, value in module.named_parameters(recurse=False):
            ...
            if _is_norm_module(module) and weight_decay_norm is not None:
                ...
                _logger.debug("Found normalization parameters: %s", qualified_name)
            if (
                _is_embedding_module(module) or _is_embedding_weight(module_param_name)
            ) and weight_decay_embedding is not None:
                hyperparams["weight_decay"] = weight_decay_embedding
                _logger.debug("Found embedding parameters: %s", qualified_name)
            if module_param_name == "param":
                hyperparams["weight_decay"] = 0.0
                _logger.debug("Found explicit parameter: %s", qualified_name)
            if callable(param_fn):
                ...
                if len(param_specifc_overrides) > 0:
                    _logger.debug(
                        "Overriding custom parameter: '%s' <- %s",
                        qualified_name,
