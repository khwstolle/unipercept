...
############################
# WANDB ACCELERATE TRACKER #
############################


class WandBTracker(WandBTrackerBase): ...
...
class WandBCallback(CallbackDispatcher):
    ...
    watch_steps: int | None = D.field(
        default_factory=lambda: laco.get_env(int, "UP_WANDB_WATCH_INTERVAL"),
        metadata={
            "help": (
    ...
    upload_config: bool = D.field(
        default_factory=lambda: laco.get_env(bool, "UP_WANDB_UPLOAD_CONFIG", default=True)
    )
    upload_code: bool = D.field(
        default_factory=lambda: laco.get_env(bool, "UP_WANDB_UPLOAD_CODE", default=False)
    )
    model_history: int = D.field(
        default_factory=lambda: laco.get_env(int, "UP_WANDB_MODEL_HISTORY", default=1)
    )
    state_history: int = D.field(
        default_factory=lambda: laco.get_env(int, "UP_WANDB_STATE_HISTORY", default=0)
    )
    inference_history: int = D.field(
        default_factory=lambda: laco.get_env(int, "UP_WANDB_INFERENCE_HISTORY", default=0)
    )
