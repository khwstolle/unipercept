...
Implements stateful classes that define the parameters of the engine, such as the
...
from unipercept.log import LOG_LEVELS, logger
...
class Precision(E.StrEnum):
    """
    Precision types, e.g. full precision or half precision.
    """
...
INFERENCE_DTYPES: T.Mapping[Precision, torch.dtype] = {
...
class EvaluationHandler(T.Protocol):
    """
    A protocol for evaluation handlers.
    """

    enabled: bool
    name: str

    def update(self, storage: T.Any, **kwargs: T.Any) -> T.Any: ...

    def compute(self, storage: T.Any, **kwargs: T.Any) -> T.Any: ...

    def plot(self, storage: T.Any, **kwargs: T.Any) -> T.Any: ...
...
class EvaluationSuite:
    ...
    handlers: dict[str, EvaluationHandler] = D.field(
    ...
    def __post_init__(self):
        ...
        if len(self.handlers) == 0:
            logger.info(
                f"Disabling evaluation suite {self.name} as it has no handlers."
            )
            self.enabled = False
...
class EngineParams:
    ...
    inference_precision: Precision = Precision.FP32
    training_precision: Precision = Precision.FP32
    ...
    @property
    def inference_dtype(self) -> torch.dtype:
        return INFERENCE_DTYPES[self.inference_precision]
