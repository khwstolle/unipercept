...
class CaptureData(Tensorclass):
    ...
    times: Tensor
    images: tensors.ImageTensor
    segmentations: tensors.PanopticTensor | None = field(
        default=None, metadata={"help": "Panoptic segmentation maps for the capture."}
    )
    depths: tensors.DepthTensor | None = field(
    ...
    def fix_subtypes_(self) -> T.Self:
        ...
        self.images = self.images.as_subclass(tensors.ImageTensor)
        self.segmentations = (
            self.segmentations.as_subclass(tensors.PanopticTensor)
            if self.segmentations is not None
            else None
        )
        self.depths = (
            self.depths.as_subclass(tensors.DepthTensor)
    ...
    def items(self):
        for k, v in self._tensordict.items():
            ...
            if k == "images":
                v = v.as_subclass(tensors.ImageTensor)
            elif k == "segmentations":
                v = v.as_subclass(tensors.PanopticTensor)
            elif k == "depths":
                v = v.as_subclass(tensors.DepthTensor)
    ...
    def fillna(self, inplace=True) -> T.Self:
        ...
        if not has_panoptic:
            self.segmentations = tensors.PanopticTensor.default(shape, device=device)
        if not has_depth:
            self.depths = tensors.DepthTensor.default(shape, device=device)
...
class InputData(Tensorclass):
    ...
    cameras: tensors.PinholeCameraTensor = field(
    ...
    def __post_init__(self):
        if self.cameras is None:
            self.cameras = tensors.PinholeCameraTensor.with_defaults_as(self.captures.images)
    ...
    def extract_frame(self, index: int) -> T.Self:
        ...
        return self.__class__(
            ...
            motions=None,
            cameras=self.cameras.clone().as_subclass(tensors.PinholeCameraTensor),
