...
CITYSCAPES_BASE_FPS: T.Final[float] = 16.7


def get_info(*args, remap_ids: bool = True, fps: float = CITYSCAPES_BASE_FPS, **kwargs):
    classes = [
        SClass(
...
class CityscapesDataset(
    PerceptionDataset,
    id="cityscapes",
    info=F.partial(get_info, remap_ids=True),
):
    """
    ...
    def _get_id2sources(self) -> Mapping[FileID, CaptureSources]:
        ...
        for file_id, file_path in map(
            ...
            sources_map[file_id] = partial_sources

        if len(sources_map) == 0:
            msg = f"Found no images in '{self.path_image}'"
        ...
        for source_key, files in sources_dict.items():
            for file_id, file_path in map(FileID.attach_id, files):
                if file_id not in sources_map:
                    raise ValueError(
                        f"File {file_path} (ID: {file_id}) does not have a corresponding image file, keys: "
                ...
                sources_map[file_id][source_key] = resource
    ...
    def _build_manifest(self) -> Manifest:
        ...
        for seq_key, ids in sequence_map.items():
            ...
            captures: list[CaptureRecord] = [
                {
                    "primary_key": f"{self.id}/{id.primary_key}",
            ...
            seq_item: ManifestSequence = {
                "camera": camera,
                "fps": CITYSCAPES_BASE_FPS / self._get_next_frame(0),
                "captures": captures,
            }
...
class CityscapesVPSDataset(
    CityscapesDataset,
    id="cityscapes-vps",
    info=F.partial(get_info, remap_ids=False),
):
    """
...
class CityscapesDVPSDataset(
    PerceptionDataset,
    id="cityscapes-dvps",
    info=F.partial(get_info, remap_ids=False, fps=CITYSCAPES_BASE_FPS / 5),
):
    """
    ...
    file structure and does not include unannotated frames.
    """
        ...
        for seq_id, seq_primkeys in seq_origins.items():
            ...
            for frame_id, sources in sorted(
                ...
                captures.append(
                    {
                        "primary_key": "/".join(
                            (self.id, "_".join(seq_primkeys[frame_id]))
                        ),
                        "sources": sources,
                        "time": float(frame_id),
                        "observer": "left",
