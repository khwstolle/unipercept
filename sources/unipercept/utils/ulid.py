...
class Timestamp(_MemView):
    ...
    * First 10 characters of ULID when in string format.
    ...
    def create(cls, timestamp: T.Self | Primitive | Datetime | ULID) -> T.Self:
        """
        Create a new :class:`ULID` instance using a timestamp value of a supported type.
...
class Randomness(_MemView):
    ...
    * Last 16 characters of ULID when in string format.
    ...
    def create(cls, randomness: T.Self | Primitive) -> T.Self:
        """
        Create a new :class:`Randomness` instance using the given randomness value of a supported type.
...
class ULID(_MemView):
    ...
    * Network byte order, big-endian, most significant bit first.
    ...
    def generate(cls) -> T.Self:
        """
        Generate a new :class:`ULID` instance using the current time and cryptographically secure
        randomness.
    ...
    def timestamp(self) -> Timestamp:
        """
        Creates a :class:`Timestamp` instance that maps to the first 48 bits of this ULID.
    ...
    def randomness(self) -> Randomness:
        """
        Creates a :class:`Randomness` instance that maps to the last 80 bits of this ULID.
