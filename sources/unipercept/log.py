...
def create_table(
    ...
    **kwargs,
    ...
    max_width = (
        int(getenv("UP_TABLE_MAX_WIDTH") or get_terminal_size((80, 20)).columns)
        if max_width is None
        else max_width
    )
    max_depth = max_depth or int(getenv("UP_TABLE_MAX_WIDTH") or 4)

    if style is None:
        style = getenv("UP_TABLE_STYLE", "rounded_outline")
    ...
    tabulate_kwargs: dict[str, T.Any] = {
        ...
        "missingval": "?",
        **kwargs,
        # "preserve_whitespace": True, # NOTE: not yet supported v0.9.0 (latest)
    }
    ...
    if isinstance(mapping, pd.DataFrame):
        ...
        mapping_to_dict := getattr(mapping, "_asdict", None)
    ) is not None and callable(mapping_to_dict):
        mapping = mapping_to_dict()

    # Determine the format of the table
    if format == TableFormat.AUTO:
        AUTO_FORMAT_MAX_KEYS = 5
        AUTO_FORMAT_MAX_DEPTH = 0
        if len(mapping) <= AUTO_FORMAT_MAX_KEYS and _depth <= AUTO_FORMAT_MAX_DEPTH:
    ...
    if format == TableFormat.WIDE:
        ...
        # If the value is a non-sequence, it will be displayed as-is
        data = []
        for k in mapping:
            v = mapping[k]
            ...
            if isinstance(v, str):
                ...
                v = [str(v_item) for v_item in v]
            else:
                v = [v]
            data.append(v)

        col_lens = [len(v) for v in data]
        pad_to = max(col_lens) if len(col_lens) > 1 else 1
        ...
        for k in mapping:
            ...
            if isinstance(v, str):
                ...
                for v_i in range(len(v_lines)):
                    ...
                    # below is achieved via `preserve_whitespace` in tabulate_kwargs
                    # if (
                    #    v_line[0] == " "
    ...
    # Preserve whitespace in the table, this can be set with the `preserve_whitespace`
    # keyword argument in tabulate, but this is not yet supported in the current version
    # of tabulate (v0.9.0). We therefore set the global variable `PRESERVE_WHITESPACE`.
    old = tabulate.PRESERVE_WHITESPACE
    tabulate.PRESERVE_WHITESPACE = True
    tbl = tabulate.tabulate(data, headers=headers, showindex=index, **tabulate_kwargs)
    tabulate.PRESERVE_WHITESPACE = old
