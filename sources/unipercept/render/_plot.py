...
def plot_predictions(
    inputs: InputData,
    outputs: T.Mapping[str, Tensor] | ModelOutput,
    /,
    info: Metadata,
    height: float = 4.0,
    scale: float = 1.0,
    ...
    seg = [
        p.get(segmentation_key, None) for p in outputs
    ]  # batch (=1) x H (=1024) x W (=2048)
    if all(s is None for s in seg):
        seg = None
        segmentation_options = SKIP
    dep = [
        p.get(depth_key, None) for p in outputs
    ]  # batch (=1) x H (=1024) x W (=2048))
...
def draw_image_segmentation(
    pan: PanopticTensor | torch.Tensor,
    info: Metadata,
    *,
